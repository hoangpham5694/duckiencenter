app.controller('ListCourseController', function($scope, $http, API,$timeout){	
	var maxRecord = 20;
	...
	$scope.txtKeyword = "";
	 var getTotalCourses = function(){
	 	$http.get(API + 'api/totalcoursejson').then(function successCallback (response){
	...
	var getListCourses = function (max, page){
		var url= API + 'api/listcoursejson?max='+max+'&page='+page+'&teacherid='+$scope.sltTeacher+'&agencyid='+$scope.sltAgency+'&keyword='+$scope.txtKeyword;
		console.log(url);
	...
	$scope.getListTeachers = function(){
		...
		var url= API + 'api/listteachersimplejson';
		console.log(url);
		$http.get(url).then(function successCallback (response){
