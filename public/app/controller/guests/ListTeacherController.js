app.controller('ListTeacherController', function($scope, $http, API,$timeout){	
	var maxRecord = 20;
	...
	$scope.keyword = "";
	 var getTotalTeachers = function(){
	 	$http.get(API + 'api/totaljson').then(function successCallback (response){
	...
	var getListTeachers = function (max, page){
		var url =API + 'api/listjson/'+max+'/'+page+"?orderby="+$scope.orderby+"&sort="+$scope.sort+"&key="+$scope.keyword;
