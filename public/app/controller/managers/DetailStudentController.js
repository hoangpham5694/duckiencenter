app.controller('DetailStudent', function($scope, $http, API,$timeout){	
	...
	$scope.getListCoursesOfStudent = function(idStudent){
		...
		var url = API + "managersites/student/listcoursesofstudentjson/"+idStudent;
	...
	$scope.modalAdd = function(){
		...
		var url = API + "managersites/course/listagenciesjson";
		$http.get(url).then(function successCallback (response){
			console.log(url);
	...
	var getListAllCourse = function(max,page){
		var url = API + "managersites/course/listjson?max="+max+"&page="+page+"&agencyid="+$scope.agencyId;
		console.log(url);
		$http.get(url).then(function successCallback (response){
	...
	$scope.deleteCourseStudent = function(courseStudentId){
		...
		if(confirmAcceptCourse){
			var url = API + 'managersites/course/deletestudentcourse/'+courseStudentId;
			$http.get(url).then(function successCallback (response){
				alert(response.data);
