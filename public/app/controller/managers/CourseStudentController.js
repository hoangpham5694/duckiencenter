...
/*	 var getTotalCourses = function(){
	...
	var getListStudent = function (id){
		var url= API + 'managersites/course/listcoursestudentsjson/'+id;
		console.log(url);
    ...
    // or server returns response with an error status.
	 ...
	 $scope.confirmDelete = function(id){
		...
		if(isConfirmDelete){
			$http.get(API + 'managersites/course/deletestudentcourse/'+id).then(function successCallback (response){
				alert(response.data);
	...
	//	console.log('id:'+id+'; keyword: '+keyword);
		var url= API + 'managersites/course/listallstudentsjson/'+id+'?keyword='+keyword;
		console.log(url);
		$http.get(url).then(function successCallback (response){
	//	getTotalCourses();
	...
	$scope.addStudentToCourse = function(courseId, studentId){
		var url= API + 'managersites/course/addstudenttosourse/'+courseId+'/'+studentId;
		console.log(url);
		$http.get(url).then(function successCallback (response){
    ...
    // or server returns response with an error status.
		...
		var url= API + 'managersites/attendance/listjson/20/1?orderby=id&sort=desc&courseid='+id;
		console.log(url);
		$http.get(url).then(function successCallback (response){
	//	getTotalCourses();
	...
	$scope.confirmAddAttendance = function(name, studyDate, state){
		...
		switch(state){
			case 'add':
				var url = API+ 'managersites/attendance/addjson?courseid='+$scope.studentCourseId+'&name='+name+'&date='+studyDateYear+'-'+studyDateMonth+'-'+studyDateDay;

			break;
			case 'edit':
				var url = API+ 'managersites/attendance/editjson?attendanceid='+$scope.idAttendance+'&name='+name+'&date='+studyDateYear+'-'+studyDateMonth+'-'+studyDateDay;
	...
	$scope.modalAttendance = function(state,id){
		...
		switch(state){
			...
			var url= API + 'managersites/attendance/attendancejson/'+id;
			console.log(url);
			$http.get(url).then(function successCallback (response){
    ...
    // or server returns response with an error status.
		...
		if(isConfirmDelete){
			$http.get(API + 'managersites/attendance/deletejson/'+id).then(function successCallback (response){
				console.log(response);
				alert(response.data);
				$scope.getListAttendances($scope.studentCourseId);
		//	
